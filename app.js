...
client.on('message', message => {
	...
	if ((message.content.match(/status/i) || message.content.match(/update/i)) && message.content.match(/test/i)) {
		...
		var apos = "'";
		var talk1 = ["Donut Mod", "Jake", "somebody", "Lucas"];
        ...
        var month = months[update.getMonth()];
        var year = update.getFullYear();
        message.channel.sendMessage("Hey everyone, this is Loren from Donut Team. Welcome to the " + month + " " + year + " Status Update." + updates[updateVariant]);
        return;
    }
